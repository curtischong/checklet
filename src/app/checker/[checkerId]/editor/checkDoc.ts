...
export const checkDocText = async (
  ...
  checkerId: CheckerId,
  user: User | null,
  onlyUseCheckId: undefined,
): Promise<FeedbackResponse | undefined> => {
  // const isUsingServer =
  //     (localStorage.getItem("modelType") ?? ModelType.GPT4o) ===
  //     ModelType.GPT4o;
  const isUsingServer = true;
  if (isUsingServer) {
    try {
  ...
  if (onlyUseCheckId) {
    ...
    const rawCheckBlueprint = await Api.getCheckBlueprint(onlyUseCheckId, user);

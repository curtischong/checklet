...
// https://chatgpt.com/share/66f375fc-e800-800e-ad82-95015a94f771
...
/**
 * Tokenizes a string into an array of words and punctuation marks.
...
function tokensToString(tokens: string[]): string {
  return tokens
    .map((token, index) => {
      // If the token is punctuation, do not add a space before it
      if (/^[.,!?;:()\-\[\]]$/.test(token) && index !== 0) {
        return token;
      } else {
...
function createCharToTokenIndexMap(text: string, tokens: string[]): number[] {
  ...
  while (charIndex < text.length && tokenIndex < tokens.length) {
    ...
    while (charIndex < text.length && /\s/.test(text[charIndex])) {
      map[charIndex] = -1; // Mark whitespace with -1
      charIndex++;
    }
...
function tokenIndexToCharIndex(
  ...
  while (charIndex < text.length && currentTokenIndex < tokenIndex) {
